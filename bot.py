...
class PolymarketFeed:
    ...
    async def _poll_markets(self):
        ...
        while self._running:
            try:
                ...
                # Fetch without tag filter — broader search, then keyword-filter locally
    ...
    async def _process_markets(self, markets_data):
        ...
        new_count = 0
        # Broad keyword match — Polymarket uses various naming conventions
        btc_keywords = ["btc", "bitcoin", "xbt"]
        time_keywords = ["5 min", "5-min", "5min", "5m ", "5 m", "minute", "up or down"]

        for m in markets_data:
            question = m.get("question", "").lower()
            # Must mention BTC and either a time indicator or be an up/down market
            is_btc = any(kw in question for kw in btc_keywords)
            is_timed = any(kw in question for kw in time_keywords)
            if not is_btc or not is_timed:
                continue
